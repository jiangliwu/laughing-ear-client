    ...
    int       nCmdShow)
{
    ...
    return EarhelloPlayer::createAndRun();
}

int EarhelloPlayer::createAndRun(void)
{
    EarhelloPlayer *host = EarhelloPlayer::sharedInstance();
    int ret = host->run();
    EarhelloPlayer::purgeSharedInstance();
    return ret;
}

EarhelloPlayer *EarhelloPlayer::s_sharedInstance = NULL;

EarhelloPlayer *EarhelloPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new EarhelloPlayer();
    }
    return s_sharedInstance;
}

void EarhelloPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
EarhelloPlayer::EarhelloPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int EarhelloPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "earhello-player");
...
void EarhelloPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void EarhelloPlayer::createViewMenu(void)
{
...
void EarhelloPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void EarhelloPlayer::updateWindowTitle(void)
{
}

void EarhelloPlayer::relaunch(void)
{
...
void EarhelloPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void EarhelloPlayer::onFileOpenProject(void)
{
...
void EarhelloPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"EarhelloPlayer");
...
void EarhelloPlayer::onFileProjectConfig(void)
{
...
void EarhelloPlayer::onFileRelaunch(void)
{
    relaunch();
}

void EarhelloPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void EarhelloPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void EarhelloPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void EarhelloPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void EarhelloPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT EarhelloPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    EarhelloPlayer *host = EarhelloPlayer::sharedInstance();
...
INT_PTR CALLBACK EarhelloPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
