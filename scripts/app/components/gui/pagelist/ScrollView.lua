...
local ScrollView = class("ScrollView", function(args)
    if not args.rect then args.rect = CCRect(0, 0, 0, 0) end
    local node = display.newClippingRegionNode(args.rect)
...
function ScrollView:ctor(rect, direction)
    ...
    self.bouncThreshold = 80
    self.defaultAnimateTime = 0.3
...
function ScrollView:onTouch(event, x, y)
    ...
    local nodePoint = self:convertToNodeSpace(CCPoint(x, y))
    if event == "began" then
        if not self.touchRect:containsPoint(nodePoint) then return false end
        return self:onTouchBegan(nodePoint.x,nodePoint.y)
    elseif event == "moved" then
        self:onTouchMoved(nodePoint.x,nodePoint.y)
    elseif event == "ended" then
        self:onTouchEnded(nodePoint.x,nodePoint.y)
    else -- cancelled
        self:onTouchCancelled(nodePoint.x,nodePoint.y)
    end
end
